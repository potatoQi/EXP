...
experiment_manager/
  core/
   ...
   scheduler/
      ...
      state_store.py     # 调度器状态与命令持久化（UI 读取，含手动停机状态字段）
  utils/
    config.py          # 配置加载与校验
...
- **状态层 (`scheduler/state_store.py`)**：为 UI/CLI 提供磁盘持久化（`scheduler_state.json` + `commands.json`），所有时间戳统一为 `Asia/Shanghai`，`summary` 中额外维护 `status_indicator`、`waiting_for_shutdown`、`shutdown_requested` 供 UI 展示关机流程。
- **实验层 (`core/experiment.py`)**：表示单个实验实例，负责目录搭建、日志记录、状态更新以及子进程生命周期管理；内置 `description` 字段用于记录自然语言描述。
- **UI 层 (`ui/`)**：`SchedulerUISession` 读取状态、下发命令；`server.py` 提供 FastAPI REST + WebSocket；`static/` 前端页面实现状态面板、详情抽屉、实时日志分页视图、状态胶囊与关机按钮；`cli.py` 封装 `EXP` 指令，负责环境检查、端口分配与浏览器打开。
...
5. 调度器循环收割已完成任务，根据返回码决定是否重试，并在结尾打印摘要；若仍有任务则继续调度。
6. 当所有任务完成且 `linger_when_idle` 为 `True`（默认），调度器会写入 `status_indicator = "awaiting_shutdown"`，保持进程存活以等待 UI 指令；点击 UI 顶部的“退出调度器”按钮会写入 `shutdown_scheduler` 命令，调度器最后回写 `status_indicator = "stopped"` 并安全退出。
7. UI/CLI（如 `EXP ./experiments`）通过 `SchedulerStateStore` 读取 `.exp_state/` 目录，展示最新状态并对命令队列写入操作，由调度器在下一轮消费。
...
- `_sync_state` 会汇总 pending/running/finished/error 队列，并把 `status_indicator`（`running` / `awaiting_shutdown` / `stopped` / `error`）、`waiting_for_shutdown`、`shutdown_requested` 写入 `summary`。新增调度状态时记得同步 UI 和文档。
...
- 时区：`metadata.json` 与日志首行时间戳统一使用 `Asia/Shanghai`，若从旧目录加载缺少 tzinfo 的时间戳需手动补齐。

### 手动停机流程速记

- `scheduler.ExperimentScheduler` 在所有任务结束后若需要保持进程活跃，请保持 `linger_when_idle = true`（默认值）。
- 当 `_waiting_for_shutdown` 为真时，`summary.status_indicator` 会报告 `awaiting_shutdown`，UI 顶部的按钮会从灰色切换到“退出调度器”。
- UI 通过 `SchedulerUISession.send_command("shutdown_scheduler", {})` 写入命令；调度器消耗后设置 `_shutdown_requested = True` 并在主循环尾部清理该标志，确保最终状态写回 `stopped`，页面文案变为“已停止”。
- 若想恢复历史行为（任务完成即退出），可在配置文件 `scheduler.linger_when_idle = false`。
...
> ⏱️ **时间策略**：底层 `SchedulerStateStore` 与 `Experiment` 都默认以 `Asia/Shanghai` 写入时间，测试用例 `tests/test_scheduler_timezones.py`、`tests/test_experiment_init.py::test_metadata_timestamp_uses_shanghai_timezone` 已覆盖该约束。
...
- **UI/CLI 扩展**：
   ...
   - 前端：`static/app.js` 负责状态轮询、详情抽屉、日志面板、状态胶囊与关机按钮；样式在 `static/styles.css` 中的 `.scheduler-status`、`.scheduler-btn`。修改后请在浏览器中手动验证，并可考虑添加端到端测试脚本。
...
- **UI 验证**：开发 UI 功能后，运行 `pytest` 以及 `EXP <实验目录>` 对照前端界面，确保 `scheduler_state.json`、`commands.json` 与 WebSocket 日志流协同工作；确认状态胶囊在「运行中 → 等待关闭指令 → 退出中 → 已停止」流程中文本与颜色正确切换。
- **测试补充**：已有单元测试覆盖 `Experiment` 初始化与时区、`SchedulerStateStore` 时区。新增调度状态或命令时务必补充测试，避免 UI 轮询与命令消费失配。
